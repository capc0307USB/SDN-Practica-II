...
import time				# Importa la librería time de Python que permite obtener obtener información y realizar operaciones con respecto al tiempo.
...
while(True):				# Bucle para actualizar el inventario constantemente cada cinco minutos.

	start_counter = time.time()							# Empieza el contador.
	...
	print("\nFue actualizado el inventario con los equipos \"wireless\" y \"appliance\" el", time.asctime(), end=".\n")	# Anuncio de la actualización del inventario y la fecha y hora en la que se realizó.

	stop_counter = time.time()								# Finaliza el contador.
	time.sleep(5*60 - (stop_counter - start_counter))		# Se esperan los cinco minutos para actualizar el inventario (5*60s - tiempo de proceso de actualización del inventario).